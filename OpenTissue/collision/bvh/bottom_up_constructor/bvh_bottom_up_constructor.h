      ...
      void run(graph_type & graph, bvh_type & bvh)
      {
        ...
        for(;node!=end;++node)
        {
          ...
          else
          {
            ...
            bv->volume() = this->fit(node->coverage().begin(),node->coverage().end(),volumes.begin(),volumes.end());
          }
        }

        this->init(graph);//--- from policy
        ...
        while(this->has_more_edges())
        {
          ...
          node->volume() = this->fit(node->coverage().begin(),node->coverage().end(),volumes.begin(),volumes.end());
          //--- Test if a BV node should be created for the new graph node
          if( this->should_create_bv(node) )  //--- from policy
          {
            node->create_bv(bvh);
