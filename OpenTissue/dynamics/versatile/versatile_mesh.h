    ...
    public:
      ...
      void initialize()
      {
        ...
        for(typename Edges::edge_iterator edge=edges.begin();edge!=edges.end();++edge)
        {
          ...
          node_type & ni = *this->node(edge->idxA());
          node_type & nj = *this->node(edge->idxB());
        ...
        for(typename Boundary::face_iterator face=boundary.begin();face!=boundary.end();++face)
        {
          ...
          node_type & ni = *this->node(face->idx0());
          node_type & nj = *this->node(face->idx1());
          node_type & nk = *this->node(face->idx2());
